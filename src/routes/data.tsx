...
const ENDPOINTS = [
  ...
  { path: "/api/public/kpis", desc: "Platform KPIs: lakes in HIGH+, alerts 30d, cases 7d, active CHWs." },
];
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-8">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        All hazard and aggregated health data is published under CC BY 4.0. Endpoints are read-only and require no
        authentication.
      </p>
      <ul className="mt-6 space-y-3">
        {ENDPOINTS.map((e) => (
          <li key={e.path} className="rounded-xl border border-border bg-card p-4">
            <a href={e.path} target="_blank" rel="noreferrer" className="font-mono text-sm text-primary hover:underline">
              GET {e.path}
            </a>
